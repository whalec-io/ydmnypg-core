...
function resolveAuthConfig(config) {
  ...
  if (!secret) {
    const mode = process.env.YS_API_MODE || process.env.NODE_ENV;
    if (mode === "production") {
      throw new Error("JWT secret is required in production. Set JWT_SECRET or config.jwt.tokenSecret.");
    }

    logger.warn("JWT secret이 안전하지 않습니다. JWT_SECRET 환경변수 또는 config.jwt.tokenSecret을 설정하세요.");
  }
...
class JwtManager {
  ...
  async verifyToken(token, refresh = false) {
    ...
    try {
      ...
      const message = err.message === "jwt malformed" ? "Invalid Token" : err.message;
