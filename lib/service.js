...
async function loadRouter(scope, options) {
  ...
  if (scope.startsWith("admin")) {
    const AdminController = await importFile(path.resolve(dirName, scope), `Admin${name}Controller`);
    if (AdminController) {
    ...
    const AdminScheduler = await importFile(path.resolve(dirName, scope), `Admin${name}Scheduler`);
    if (AdminScheduler) {
    ...
    const Scheduler = await importFile(path.resolve(dirName, scope), `${name}Scheduler`);
    if (Scheduler) {
      const sched = new Scheduler(service);
      sched.generateJobs({ service, jobs: await Scheduler.jobs(sched) });
    }
  } else {
    const Controller = await importFile(path.resolve(dirName, scope), `${name}Controller`);
    if (Controller) {
...
class Service {
  get path() {
    if (this.type === "api") {
      if (this.context.path) {
        return "/" + [this.context.path, this.interfaceVersion, this.basePath].join("/");
      }
  ...
  constructor(name, options) {
    const { type, config, router, path, dir, dirname, scopes, contentType, interfaceVersion, main } = options;
    ...
    this.interfaceVersion = interfaceVersion || this.context.interfaceVersion || "v1";
    this.main = main === true;
