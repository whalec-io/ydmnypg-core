...
class Model {
  ...
  setCollection(col) {
    if (this.design.dbConnection !== (col.dbConnection && col.dbConnection.key)) {
      logger.warn("connection mismatch", this.design.dbConnection, col.dbConnection && col.dbConnection.key);
      return;
    }
    this.collection = col;
  }
  ...
  filterData(origin, options = {}) {
    ...
    try {
      ...
      res = options.excludesAll === true ? {} : JSON.parse(JSON.stringify(origin));
    }
  ...
  #parseDbArgs(modelsOrCallback, callback) {
    ...
    const models = Array.isArray(modelsOrCallback) ? modelsOrCallback : [modelsOrCallback];
  ...
  async withDb(modelsOrCallback, callback) {
    ...
    if (!this.db) {
      ...
      throw new Error(`withDb: DB connection not found (${this.constructor.name})`);
    }
  ...
  async withTransaction(modelsOrCallback, callback) {
    ...
    if (!this.db) {
      throw new Error(`withTransaction: DB connection not found (${this.constructor.name})`);
    }
