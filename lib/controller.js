...
class Controller {
  ...
  checkFileParams(request, response, params, next) {
    ...
    if (fields.length > 0) {
      return process.framework.upload
        .fields(fields)
        .call(this, request, response, next);
    }
  ...
  parseDeviceInfo(request, response, next) {
    if (request.headers["x-device-info"]) {
      try {
        request.deviceInfo = JSON.parse(
          decodeURIComponent(request.headers["x-device-info"]),
        );
      } catch {
        request.deviceInfo = {};
      }
  ...
  generateRouters(options) {
    ...
    for (const routerOption of routers) {
      const { controllers, params, permissions, responseOptions } =
        routerOption;
      const routePath = routerOption.path;
      const method =
        (routerOption.method && routerOption.method.toLowerCase()) || "get";
      ...
      const args = [
        path.join(options.path || "", routePath),
        ...middlewares,
        ...handlers,
      ];
      router[method].apply(router, args);

      routerOption.routePath = path.join(
        this.service.path,
        options.path || "",
        routePath,
      );
      routerOption.group = this.service.name;
      routerOption.scopes = _.uniq([
  ...
  resolveContentType(response) {
    const contentType = (
      response.get("Content-Type") || this.contentType
    ).toLowerCase();
  ...
  onUnauthorized(response, errorMessage = "Unauthorized Error") {
    return this.onError(
      response,
      this.createError(errorMessage, { statusCode: 401 }),
      {},
    );
  }
  ...
  static async autoLoad(cwd, options = {}) {
    ...
    return [
      ...priorityHandlers.high,
      ...priorityHandlers.medium,
      ...priorityHandlers.low,
    ];
  }
}
