...
class SwaggerBuilder {
  ...
  getDescription(param) {
    ...
    if (param.minLength !== undefined) desc += ` (minLength: ${param.minLength})`;
    if (param.maxLength !== undefined) desc += ` (maxLength: ${param.maxLength})`;
  ...
  pathInfo(route, info, scope = "") {
    ...
    const requestType = isMultipart ? "multipart/form-data" : "application/x-www-form-urlencoded";
  ...
  dbTypeToSwagger(attribute) {
    ...
    if (attribute.type === "tinyint" && attribute.length === 1) {
      type = "boolean";
    }
  ...
  getSchemaInfos(model) {
    const modelObjects = [];
    if (!model.design || !model.design.schemas || model.design.schemas.length === 0) {
      return modelObjects;
    }
    ...
    for (const schema of model.design.schemas) {
      ...
      modelObjects.push({
        name: schema.default === true ? model.name : `${model.name}.${schema.name}`,
  ...
  buildJSON(options = {}, scope = "") {
    ...
    const host = this.context.host || this.config.host || `${getLocalIp()}:${this.context.port}`;
    const modelObjects = this.buildModels(models, scope);

    return {
      swagger: "2.0",
      info: {
        title: this.config.title || `${this.context.title} (${this.context.config.mode})`,
